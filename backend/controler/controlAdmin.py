    ...
    if session.get("logged_in"):
        if session["admin"] !=  0:
            if request.method == "POST":
                ...
                print(decode(requests.post(f"{URL}{BASE}/registInfos/", 
                    encode(token))))
...
def delInfos(id):
    if session.get("logged_in"):
        if session["admin"] !=  0:
            ...
            print(decode(requests.post(f"{URL}{BASE}/delInfos/", 
                encode(token))))
    ...
    if session.get("logged_in"):
        if session["admin"] !=  0:
            if request.method == "POST":
                ...
                print(decode(requests.post(f"{URL}{BASE}/registCours/", 
                    encode(token))))
...
def delCours(id):
    if session.get("logged_in"):
        if session["admin"] !=  0:
            ...
            print(decode(requests.post(f"{URL}{BASE}/delCours/", 
                encode(token))))
...
def presentAccounts():
    if session.get("logged_in"):
        if session["admin"] !=  0:
            if request.method == "POST":
                print(decode(requests.post(f"{URL}{BASE}/presentAccounts/", 
...
def presentAccount():
    if session.get("logged_in"):
        if request.method == "POST":
            print(decode(requests.post(f"{URL}{BASE}/presentAccount/", 
                encode({"account" : session["account"]}))))
