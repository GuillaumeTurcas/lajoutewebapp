...
def homepage():
    if session.get("logged_in"):
        ...
        infos = decode(requests.post(f"{URL}{BASE}/getInfos/",
...
def calendrier():
    if session.get("logged_in"): 
        cours = decode(requests.post(f"{URL}{BASE}/getCours/",
            encode(session["account"])))
...
def application():
    if session.get("logged_in"):
        ...
        contacts = decode(requests.post(f"{URL}{BASE}/getAccounts/", 
            encode(session["account"])))
...
def matchs():
    if session.get("logged_in"):
        ...
        matchs = decode(requests.post(f"{URL}{BASE}/getMatchs/", 
            encode(token)))
...
def sujet():
    if session.get("logged_in"):
        if session["admin"] !=  0:
            ...
            sujets = decode(requests.post(f"{URL}{BASE}/getSujets/", 
                encode(token)))
...
def config():
    if session.get("logged_in"):
        if session["admin"] > 2:
            ...
            config = decode(requests.post(f"{URL}{BASE}/getConfigs/", 
                encode(token)))
